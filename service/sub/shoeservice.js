...
function packageAttr(detail) {
    return [{
        ...
        key: '材 质',
        value: detail.material.material_name
    }, {
        key: '备 注',
        value: detail.note
    }, {
...
exports.findSearchConditions = function(shopId, param, callback) {
    async.waterfall([
        function(cb) {
            Shoe.findAll({
                attributes: [
                    [Sequelize.fn('COUNT', 'brand_id'), 'total']
                ],
                include: [{
                    model: ShoeBrand,
                    as: 'brand',
                ...
                for (var i = 0; i < result.length; i++) {
                    ...
                    arr.push({
                        name: result[i].brand.brand_name,
                        id: result[i].brand.id,
        ...
        function(data, cb) {
            Shoe.findAll({
                attributes: [
                    [Sequelize.fn('COUNT', 'material_id'), 'total']
                ],
                include: [{
                    model: ShoeMaterial,
                    as: 'material',
                ...
                for (var i = 0; i < result.length; i++) {
                    arr.push({
                        name: result[i].material.material_name,
                        id: result[i].material.id,
                        total: result[i].dataValues.total
                    });
                }
                cb(null, {
        ...
        for (var i = 0; i < result[0].length; i++) {
            ...
            if (result[0][i].id == parseInt(curBrand)) {
                ...
                currentSelected.push({
                    ...
                    link: 'a' + param.a + 'b0' + 'c' + curMaterial + 'd' + curColor
                });
            }
        }
        ...
        for (var i = 0; i < result[1].length; i++) {
            ...
            if (result[1][i].id == parseInt(curMaterial)) {
                ...
                currentSelected.push({
                    ...
                    link: 'a' + param.a + 'b' + curBrand + 'c0' + 'd' + curColor
                });
            }
        }
        for (var i = 0; i < result[2].length; i++) {
            ...
            if (result[2][i].id == parseInt(curColor)) {
                ...
                currentSelected.push({
                    ...
                    link: 'a' + param.a + 'b' + curBrand + 'c' + curMaterial + 'd0'
                });
            }
        }
        callback({
            searchConditions: result,
...
exports.findFullConditions = function(callback) {
    async.waterfall([
        ...
        arr.push({
            name: '外部链接',
            alias: 'rel_link',
            desc: '如果您在其他地方也有网店, 输入此链接可以跳转到该链接上',
            type: 'string'
        });
        arr.push({
            name: '宝贝图片',
...
function batchSave(arr, callback) {
    Shoe.bulkCreate(arr, {
        fields: ['shop_id', 'user_id', 'brand_id', 'price', 'raw_price', 'short_name', 'description', 'from_country_id', 'material_id', 'note', 'serial_number', 'rel_link']
    }).complete(function(err, result) {
        callback(result);
    })
}

function save(shop, callback) {
    Model.Shoe.create(shop).complete(function(err, result) {
        if (err) {
            callback(0);
        } else {
            callback({
